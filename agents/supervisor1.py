...
from agents.insightAgent import generate_insights  # This returns InsightResult now
from agents.visualizerAgent import visualize_data  # This returns VisualizerResult


# In insights_node function, UPDATE to handle InsightResult object:
...
def insights_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        # Call generate_insights - it returns InsightResult object
        insights = generate_insights(
            csv_path=state["csv_path"],
        ...
        # InsightResult has: summary, details, dataframe_path
        if not insights:
            raise ValidationError("Insights generation returned no results")
...
# In visualize_node function, UPDATE to handle VisualizerResult object:
...
def visualize_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        # Call visualize_data - it returns VisualizerResult object
        ...
        # VisualizerResult has: summary, chart_path, chart_type, execution_status, data_points
        if not chart or chart.execution_status != "success":
...
# ============================================================
# FILE 3: integrations/main.py (NO CHANGES NEEDED)
# ============================================================
# main.py doesn't directly import from insightAgent or visualizerAgent
# It only imports from supervisor, so NO CHANGES required


# ============================================================
# FILE 4: integrations/app.py (NO CHANGES NEEDED)
# ============================================================
# app.py only imports process_query from main.py
# It doesn't directly import from insightAgent or visualizerAgent
...
# ============================================================
"""
1. insightAgent.py (NEW):
   - Added InsightResult model for return type compatibility
   - generate_insights() now returns InsightResult object with:
     - summary: str
     - details: dict
     - dataframe_path: str
   - Includes all forecasting, anomaly detection, correlation features

2. supervisor.py (UPDATED):
   - insights_node(): Updated to handle InsightResult object
   - visualize_node(): Updated to handle VisualizerResult object
   - No other changes needed

3. main.py (NO CHANGES):
   - Doesn't import from insight/visualizer agents directly
   
4. app.py (NO CHANGES):
   - Only imports from main.py

KEY POINTS:
- InsightResult has .summary, .details, .dataframe_path attributes
- VisualizerResult has .summary, .chart_path, .chart_type, .execution_status, .data_points
- Both are Pydantic models for type safety
- All functionality preserved, just proper return types added
"""