...
from agents.visualizerAgent import visualize_trend
from agents.knowledge import get_finops_tip
...
def data_fetcher_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        logger.info("[DataFetcher] Loading CSV into SQLite memory DB...")
...
5. Use GROUP BY and ORDER BY as needed
6. Avoid markdown formatting
7. Handle filters, trends, and grouping logically
"""
        ...
        # --- Format results ---
        if result_df.empty:
            response_text = "⚠️ Query executed successfully but returned no results."
        else:
...
def visualize_node(state: AgentState) -> AgentState:
    try:
        ...
        chart = visualize_trend(state["dataframe_path"])
...
def knowledge_node(state: AgentState) -> AgentState:
    try:
        ...
        # Handle LangChain StructuredTool properly
        try:
            if hasattr(get_finops_tip, 'invoke'):
                tip = get_finops_tip.invoke({"query": state["original_query"]})
            elif hasattr(get_finops_tip, 'run'):
                tip = get_finops_tip.run(state["original_query"])
            else:
                tip = get_finops_tip(state["original_query"])
            
            response_text = f"{current_response}\n\n💡 FinOps Tip: {tip}"
        except Exception as tip_error:
            logger.error(f"Error generating tip: {tip_error}")
            # Don't add tip if it failed
            response_text = current_response
        
        result = {
            **state,
            "tip": tip if 'tip' in locals() else None,
            "response": response_text
        }
...
def run_supervisor(state: dict, csv_path: str):
    ...
    try:
        ...
        if not final_response or final_response == "None" or final_response is None:
            final_response = "⚠️ Pipeline completed but no response was generated."
